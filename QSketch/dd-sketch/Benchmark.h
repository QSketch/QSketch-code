...
public:
	...
	std::pair<std::pair<double, double>, double> Run(double w, double tower, uint32_t threshold, uint32_t memory, uint32_t d, uint32_t mm) {
        Init();
        ...
        for (int i = 0; i < num; i++)
        {
            ans[i] = dd.query(qrys[i], query_quantile);
        }
