...
LCU_type * LCU_Init(float fPhi, double qeps, long long samples_num)
{
	int i;
	int k = 1 + (int) 1.0/fPhi;

	//std::cout << "Initialize space saving, #entries: " << k << ", qsketch eps: "<< qeps << ", #samples: " << samples_num << std::endl;
	...
	result->k=k;
	result->n=0;
	result->qsketch_eps =  (double)qeps;
	result->samples_num = samples_num;
	...
	for (i=0;i<k;i++) 
	{
		...
		result->items[i].qsketch = new GK<double>(result->qsketch_eps);
		result->items[i].timestamp = 0;
...
int LCU_ComputeMem(LCU_type * lcu)
{
	int entriesNum = 0;
	int mem = 0;
	//int samples_mem = lcu->samples_num * 8;
	int samples_mem = lcu->samples_num * 2;

	//int flag = 1;

	for (int i=0; i<lcu->k; ++i) {
		//if (flag) {
			std::cout << i << " gk sketch memory: " << lcu->items[i].qsketch->GetEnryMapSize()*12 << std::endl;
		//	flag = 0;
		//}
		entriesNum += lcu->items[i].qsketch->GetEnryMapSize();
	}


	mem = entriesNum* 12; // Each enrty is of size 12 byte
	std::cout << "entries in ss taking in account "<< lcu->k << ", sketch mem:[Mbyte] "<< mem / (double) (pow (10.0, 6.0)) << std::endl;

	return (mem + samples_mem);
}
...
LCUITEM * LCU_GetNewCounter(LCU_type * lcu) {
	...
	// Get a new quantile sketch
	delete (newi->qsketch);
    newi->qsketch = new GK<double>(lcu->qsketch_eps);
	newi->timestamp = ts;
	return (newi);
}
...
void LCU_addToSample(LCU_type * lcu, int newitem, double latency) {
	++lcu->nb_processed;
	sampleEntry se;
	se.flow = newitem;
	se.latency = latency;
  ...
  if (lcu->samples_num >= lcu->nb_processed) {
      lcu->samples.push_back(se);
  } else {
      unsigned long long j = rand() % (lcu->nb_processed);
      if (j < lcu->samples_num) {
        lcu->samples[j] = se;
      }
  }
}

void LCU_UpdateLatency(LCU_type * lcu, int newitem, double latency) {
	int h;
	LCUITEM *il;
	...
	LCU_addToSample(lcu, newitem, latency);
}
...
double LCU_QuantileQuery(LCU_type * lcu, int newitem, double rank) {
	int h;
	LCUITEM *il;
	...
	il->qsketch->finalize();
	double val = il->qsketch->query_for_value(rank);
  	return val;
}

double LCU_QuantileQuerywGsamples(LCU_type * lcu, int newitem, double rank) {
	int h;
	LCUITEM *il;
	std::vector<double> sample_buff;
	GK<double>* tmpsketch = new GK<double>(lcu->qsketch_eps);
	...
	il->qsketch->copySketch(tmpsketch);

	for(auto const& value: lcu->samples) {
		if (value.flow == newitem && value.ts <= il->timestamp) {
			//std::cout << "To sample buffer:! flow: " << value.flow <<" latency: " <<value.latency << " ts: " << value.ts << std::endl;
			sample_buff.push_back(value.latency);
			tmpsketch->feed(value.latency);
		}
	}


	tmpsketch->finalize();
	double val = tmpsketch->query_for_value(rank);
  	return val;
}
