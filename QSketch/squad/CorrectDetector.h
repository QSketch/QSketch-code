...
public:
	...
	double query_quantile(ID_TYPE id, uint64_t time) {
		sort(record[id].begin(), record[id].end());
		int index = lower_bound(record[id].begin(), record[id].end(), time) - record[id].begin();
		double quantile = 1.0 * index / record[id].size();
		return quantile;
	}
