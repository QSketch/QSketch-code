...
public:
	...
	void insert(ID_TYPE id, DATA_TYPE time) {
		...
		for (int i = 0; i < d; ++i) {
			...
			frequency[i][index]++;
			if (latency[i][index][0] == 0) {
				flag = 0;
				for (int j = 1; j < M; j++)
				{
					if (latency[i][index][j] > time) 
					{
						latency[i][index][j - 1] = time; 
						flag = 1;
						break;
					}
					latency[i][index][j - 1] = latency[i][index][j];
				}
				if (!flag) latency[i][index][M - 1] = time;
			}
			else {
				if (rand() % 100 < 100 * quantile) {
					// replace minimum value
					if (time < latency[i][index][0]) continue;
					flag = 0;
					for (int j = 1; j < M; j++)
					{
						if (latency[i][index][j] > time) 
					...
					if (time > latency[i][index][M - 1]) continue;
					flag = 0;
					for (int j = M - 2; j >= 0; j--)
					{
						if (latency[i][index][j] < time) 
					...
					if (!flag) latency[i][index][0] = time;
				}
			}
		}
	}
