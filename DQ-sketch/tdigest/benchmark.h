...
public:
	...
	std::pair<double, double> Run(double w, uint32_t memory, double compression) {
            ...
            for (int j = head[g]; j; j = nxt[j])
            {
                if (tid[j] == dataset[i].id)
                {
                    ...
                    if (dataset[i].timestamp > last_time[j])
                    {
                        ...
                        tt = clock();
				        td.insert(dataset[i].id, dataset[i].timestamp - last_time[j]);
                        tottime += (clock() - tt);
                    }
        ...
        for (int i = 1; i <= cnt; i++) {
            ...
            uint64_t predict_td = td.query(tid[i], query_quantile);
            double predict_quantile_td = gt.query(tid[i], predict_td);
            error_td += abs(predict_quantile_td - query_quantile);
            //std::cout << predict_quantile_td << "\n";
        }

        return std::make_pair(throughput, error_td / num);
	}
...
public:
	...
	std::pair<double, double> Run(double w, uint32_t memory, double compression) {
            ...
            for (int j = head[g]; j; j = nxt[j])
            {
                if (tid[j] == dataset[i].id)
                {
                    ...
                    if (i > last_time[j])
                    {
                        ...
                        tt = clock();
				        td.insert(dataset[i].id, i - last_time[j]);
                        tottime += (clock() - tt);
                    }
                    last_time[j] = i;
                    break;
		...
		//std::cout << "Average Error (TD): " << error_td / num << "\n";
        return std::make_pair(throughput, error_td / num);
	}
		...
		for (int i = 0; i < run_length; ++i) {
            ...
            for (int j = head[g]; j; j = nxt[j])
            {
                if (tid[j] == dataset[i].unit_id)
                {
                    ...
                    tt = clock();
				    td.insert(dataset[i].unit_id, dataset[i].fetch_time);
                    tottime += (clock() - tt);
                    break;
                }
            }
