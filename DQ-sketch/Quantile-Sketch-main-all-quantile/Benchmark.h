...
public:
	...
	void Run(double tower, uint32_t threshold, uint32_t memory, uint32_t d, uint32_t mm, double *res, double *tput, int &cnt1, int &cnt2) {
        Init();
        ...
        tput[++cnt2] = throughput;

		//std::cout << totaltime << std::endl;

		double error_qs = 0;
        int num = 0;
        for (double w = 0.1; w < 0.95; w += 0.1)
        {
            error_qs = 0; num = 0;
            for (int i = 1; i <= cnt; i++) 
            {
                ...
                uint64_t predict_qs = qs.query(tid[i], w);
                double predict_quantile_qs = gt.query(tid[i], predict_qs);
                error_qs += abs(predict_quantile_qs - w);

                //std::cout << predict_qs << " " << predict_quantile_qs << " " << w << std::endl;
                //std::cout << predict_quantile_qs << "\n";
            }
            res[++cnt1] = error_qs / num;            
        }
	}
